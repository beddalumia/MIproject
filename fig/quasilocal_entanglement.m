...
U4 = load('U_list.txt');
S4 = QcmP.post.eentropy_line('U','2sites');
...
xlabel("$U/D$")
ylabel("$s_{\langle ij\rangle}/\log(2)$")
set(gca,'FontSize',15)
legend([...%"$1\times2$, $N_\mathrm{bath}=10$ (ED)",...
        "$2\times2$, $N_\mathrm{bath}=08$ (ED)",...
        ...%"$1\times2$, $N_\mathrm{bath}=12$ (ASCI)",...
        "$2\times2$, $N_\mathrm{bath}=24$ (ASCI)",...
        "$3\times2$, $N_\mathrm{bath}=36$ (ASCI)",...
